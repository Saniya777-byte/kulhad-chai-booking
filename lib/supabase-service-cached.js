...
// Helper to wrap read operations with caching
const withCache = (cacheType, fetchFunction) => {
  return async (...args) => {
...
// Helper to wrap write operations with cache invalidation
const withCacheInvalidation = (cacheType, writeFunction) => {
  return async (...args) => {
    try {
      ...
      console.error(`Error in write operation for ${cacheType}:`, error);
      throw error;
    }
  };
};
...
// Business Settings operations - cached
...
// Helper functions - these don't need caching as they use cached data internally
